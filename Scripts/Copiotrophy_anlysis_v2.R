...
#Filter non-het ASV
plot_df <- plot_df %>% 
  subset(!Jesse_aggname %in%c("Nitroso","Nitrosp")) 
...
write.csv(plot_df,file="Genome_guild_stats.csv")
write.csv(plot_df,file="Copiotrophy_index_pergenome.csv")

plot_df %>% group_by(Jesse_aggname) %>% summarise(IoC=median(PC1t))
