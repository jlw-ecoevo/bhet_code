...
asv[,3:129] <- t(t(asv[,-c(1:2)])/colSums(asv[,-c(1:2)]))
...
load("gtdb220_genome_predictions.rda")
pred_df <- pred_df %>% subset(transect=="I8I9")
...
abline(v=3.14)
abline(h=0.05)
c_l <- max(10^mC$data[mC$uncertainty<0.05 & 10^mC$data<3])
c_h <- min(10^mC$data[mC$uncertainty<0.05 & 10^mC$data>3])
...
setwd("C:/Users/jlwei/Documents/bhet_code/Figures")
pdf("Copiotrophy_ridges_I8I9.pdf",width=10,height=10)
pCr
dev.off()


#Write output ASV and IoC data, along with IoC inputs
CI <- plot_df %>% subset(select=c(Accession,PC1t,dCUB,nGenes,nCAZy))
names(CI)[5] <- "nCAZy"
names(asvg) <- c("ASV","Accession")
asv_CI <- merge.easy(asvg,CI,key="Accession") %>%
  subset(!is.na(PC1t)) %>%
  subset(select=c(ASV,PC1t,dCUB,nGenes,nCAZy))
names(asv_CI)[2] <- "CopiotrophyIndex"
setwd("C:/Users/jlwei/Documents/bhet_code/Data")
write.table(asv_CI,file="Copiotrophy_index_I8I9.tsv")
write.csv(asv_CI,file="Copiotrophy_index_I8I9.csv")
